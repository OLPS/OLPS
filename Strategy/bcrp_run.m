...
if (exist('OCTAVE_VERSION', 'builtin'))
  [day_weight] = sqp(x0, @(x)(-prod(data*x)), @(x)(1-sum(x)), [], 0, 1);
else
  options = optimset('largescale', 'off', 'Display', 'off', 'Algorithm', 'active-set');
  [day_weight, ~]=fmincon(@(day_weight)(-prod(data*day_weight)), x0, A, b, Aeq, beq, lb, ub,[], options);
end
...
% Debug Information
fprintf(fid, 'BCRP(tc=%.4f), Final return: %.2f\n', tc, cum_ret);
fprintf(fid, '-------------------------------------\n');
fprintf(1,  'BCRP(tc=%.4f), Final return: %.2f\n', tc, cum_ret);
fprintf(1, '-------------------------------------\n');
end