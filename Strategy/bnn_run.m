...
% This program simulates the BNN strategy on data
%
% function [cum_ret, cumprod_ret, daily_ret, daily_portfolio, exp_ret] ...
%    = bnn_run(fid, data, K, L, tc, opts)
%
...
fprintf(fid, '-------------------------------------\n');
if (~opts.quiet_mode)
    fprintf(fid, 'Parameters [K=%d, L=%d, tc=%f\n]', K, L, tc);
...
for t = 1:1:n,
    ...
    if (t >=2)
        [day_weight, exp_w] ...
            = bnn_kernel(data(1:t-1, :), K, L, exp_ret, exp_w);
    end
...
% Debug Information
fprintf(fid, 'BNN(%d, %d, %.4f), Final return: %.2f\n', ...
    K, L, tc, cum_ret);

fprintf(fid, 'BNN, Experts return:\n');
fprintf(fid, '%f\t', exp_ret);
fprintf(fid, '\n');
fprintf(fid, '-------------------------------------\n');

fprintf(1, 'BNN(%d, %d, %.4f), Final return: %.2f\n', ...
    K, L, tc, cum_ret);
if (~opts.quiet_mode)
    fprintf(1, 'BNN, Experts return:\n');
    fprintf(1, '%f\t', exp_ret);
    fprintf(1, '\n');
end
fprintf(fid, '-------------------------------------\n');
